...
class ContactEdit extends Component {
  ...
  async componentDidMount() {
    if (this.props.match.params.id !== 'new') {
      const contact = await (await fetch(`/api/contact/${this.props.match.params.id}`)).json();
  ...
  async handleSubmit(event) {
    ...
    await fetch('/api/contact' + (item.id ? '/' + item.id : '') , {
  ...
  render() {
    ...
    const title = <h2>{item.id ? 'Edit Contct' : 'Add Contact'}</h2>;
